...
export default function () {
  ...
  const user = {
      ...
      nome : `${numeroAleatorioUsuarios}` + " " + `${numeroAleatorioProdutos}` + " " + `${(numeroAleatorioUsuarios+numeroAleatorioProdutos)}`,
      email:   `${numeroAleatorioUsuarios}` + `${numeroAleatorioProdutos}` +  `${(numeroAleatorioUsuarios+numeroAleatorioProdutos)}` + "@qa.com",
      password: "teste",
      administrador: "true"
  ...
  const payload_login = {
      ...
      "email": user.email, //Necessita fazer um cadastro antes do usuario
      "password": "teste",
  }
